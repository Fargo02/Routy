...
class IosPersistentFiles : PersistentFiles {
    private val directory = (NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, true).first() as String) + "/Routy"
    init {
    ...
    override suspend fun read(name: String): String? = withContext(Dispatchers.Default) {
        ...
        if (!NSFileManager.defaultManager.fileExistsAtPath(path)) null
        else requireNotNull(NSString.stringWithContentsOfFile(path, NSUTF8StringEncoding, null))
    }
