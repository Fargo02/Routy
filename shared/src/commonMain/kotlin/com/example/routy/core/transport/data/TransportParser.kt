...
class TransportParser(
    ...
    fun network(payload: String): TransportNetwork {
        ...
        val stops =
            dto.busStops.map { (key, stop) ->
                require(key.isNotBlank() && key == stop.id)
                BusStop(
                    key,
                    stop.number,
                    LocalizedName(stop.english, stop.georgian, stop.original),
                    GeoPoint(stop.latitude, stop.longitude),
                    stop.routes.map { (routeId, service) ->
    ...
    fun vehicles(
        ...
        json
            .decodeFromString<VehiclesResponse>(payload)
            .data
            .map {
