...
@Serializable
data class CachedDatabase(val payload: String, val updatedAtMillis: Long, val schemaVersion: Int = 1)
...
class FileTransportLocalDataSource(private val files: PersistentFiles) : TransportLocalDataSource {
    override suspend fun read(): CachedDatabase? = files.read("transport-v1.json")?.let {
    ...
    override suspend fun write(database: CachedDatabase) =
        files.write("transport-v1.json", Json.encodeToString(database))
}
