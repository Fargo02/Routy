...
fun FavoritesScreen(
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item { StatusPanel(state.network) { model.accept(FavoritesIntent.Retry) } }
        if (state.routes.isEmpty() && state.stops.isEmpty() && state.network.network != null) item { EmptyPanel(TextKey.NoFavorites) }
        items(
            state.routes,
            key = { "route:${it.id}" },
        ) { route -> RouteCard(route, { model.accept(FavoritesIntent.SelectRoute(route.id)) }) }
        items(state.stops, key = { "stop:${it.id}" }) { stop -> StopCard(stop, { model.accept(FavoritesIntent.SelectStop(stop.id)) }) }
    }
}
